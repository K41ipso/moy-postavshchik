...
export default function CalendarView({ orders, onClose, onRemoveOrder, onOpenSupplier }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
      ...
      <div 
        className={`relative bg-white rounded-2xl shadow-2xl w-full max-w-7xl h-[600px] sm:h-[650px] overflow-hidden flex flex-col sm:flex-row transition-all duration-300 ease-out ${
          isVisible 
      ...
      >
        ...
        <div className="w-full sm:flex-1 flex flex-col border-b sm:border-b-0 sm:border-r border-gray-200">
          <div className="bg-gradient-to-r from-blue-500 to-indigo-500 p-4 sm:p-5 text-white shrink-0">
            <div className="flex justify-between items-start">
              <div>
                <h2 className="text-xl sm:text-2xl font-bold">Календарь заказов</h2>
                <p className="text-white/90 text-xs sm:text-sm">{orders.length} запланированных поставок</p>
              </div>
          ...
          <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100 shrink-0">
            ...
            <h3 className="text-base sm:text-lg font-bold text-gray-900">{monthNames[month]} {year}</h3>
          ...
          <div className="flex-1 p-4 sm:p-6 flex flex-col">
            <div className="grid grid-cols-7 gap-1 sm:gap-2 mb-2 sm:mb-3 shrink-0">
              {dayNames.map(day => (
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <button
                    ...
                    className={`min-h-0 rounded-lg border-2 transition-all duration-200 relative flex flex-col items-center justify-center text-xs sm:text-sm ${
                      isSelected
        ...
        <div className="w-full sm:w-[400px] flex flex-col bg-gray-50 border-t sm:border-t-0">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {totalOrders === 0 ? (
              ...
              displayOrders.map(order => (
                <div 
                  key={order.id} 
                  className="bg-white p-3 sm:p-4 rounded-xl border border-gray-200 hover:shadow-md transition-shadow group cursor-pointer"
                  onClick={() => {
                    if (onOpenSupplier) {
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1 min-w-0 pr-2">
                      <p className="font-semibold text-gray-900 truncate text-xs sm:text-sm">{order.supplierName}</p>
                      <p className="text-[10px] sm:text-xs text-gray-500 mt-0.5">{order.category}</p>
                  ...
                  {order.comment && (
                    <p className="text-[10px] sm:text-xs text-gray-500 mt-2 italic bg-gray-50 p-1.5 sm:p-2 rounded">"{order.comment}"</p>
                  )}
                </div>
              ))
            )}
