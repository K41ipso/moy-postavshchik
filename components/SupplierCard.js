...
export default function SupplierCard({ supplier, onClick }) {
  ...
  const handleFavoriteClick = (e) => {
    ...
    if (typeof window !== 'undefined') {
      ...
      // ВАЖНО: Сообщаем всем компонентам об изменении
      window.dispatchEvent(new Event('favoritesChanged'));
    }
  };

  return (
    // СОХРАНЯЕМ scale, но делаем скругления строже (rounded-xl вместо 2xl)
    <div 
      onClick={() => onClick && onClick(supplier)}
      className={`bg-white p-5 rounded-xl shadow-sm border-2 transition-all duration-300 cursor-pointer hover:shadow-xl hover:scale-[1.02] ${
        isBestMatch ? 'border-green-400 shadow-[0_0_15px_rgba(34,197,94,0.2)]' : 'border-gray-100 hover:border-orange-200'
      }`}
    >
      <div className="flex justify-between items-start mb-3">
        <div className="flex-1 min-w-0 pr-3">
          <h3 className="text-lg font-bold text-gray-900 truncate leading-tight">{supplier.name}</h3>
          <p className="text-xs text-gray-500 mt-1 font-medium">{supplier.category} • {supplier.subCategory}</p>
        </div>
        
        {matchInfo && (
          <div className={`px-2.5 py-1 rounded-md text-[10px] font-bold border flex-shrink-0 uppercase tracking-wide ${matchInfo.color}`}>
            {matchInfo.text}
          </div>
        )}
        ...
        >
          <svg className="w-5 h-5" fill={isMounted && isFavorite ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
          </svg>
        </button>
      </div>

      {/* РАЗГРУЖЕННАЯ СЕТКА: Только самое важное */}
      ...
      {/* Сертификаты теперь только маленьким тегом, если они есть, чтобы не занимать много места */}
      ...
      {/* ФУТЕР КАРТОЧКИ: Разделяем важность кнопок */}
      <div className="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {/* "Подробнее" делаем простой текстовой ссылкой, чтобы не конкурировать с главной кнопкой */}
          <span className="text-sm text-gray-500 font-medium hover:text-orange-600 transition-colors cursor-pointer">
            Подробнее →
          </span>
          {/* "Связаться" оставляем яркой кнопкой (CTA) */}
          <a
            href={`tel:${supplier.contact.replace(/\D/g, '')}`}
            onClick={(e) => e.stopPropagation()}
            className="px-4 py-2 bg-orange-500 text-white text-sm font-semibold rounded-lg hover:bg-orange-600 transition-colors flex items-center gap-1.5 shadow-sm hover:shadow-md"
          >
